...
export default function SectionBlock({ section }) {
  ...
  return (
    <section
      id={`section-${id}`}
      className="relative isolate h-[100dvh] py-16 sm:py-24 bg-transparent w-full mt-20 flex flex-col items-center justify-center px-6"
    >
      ...
      {text && (
        <motion.div initial={{ opacity: 0, y: 40 }}
          ...
          {Array.isArray(text)
            ? text.map((t, idx) => <p key={idx} className="text-white/90">{t}</p>)
